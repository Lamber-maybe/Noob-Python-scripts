...
# 读取domain.txt文件中的域名，并进行URL安全的base64编码
with open('domain.txt', 'r', encoding='utf-8') as file:
    domains = [base64.urlsafe_b64encode(line.strip().encode('utf-8')).decode('utf-8') for line in file]
...
for domain in tqdm(domains, desc="Processing"):
    url = f"https://hunter.qianxin.com/openApi/search?api-key={api_key}&search={domain}&page=1&page_size=1&is_web=1&start_time=2024-01-01&end_time=2024-09-01"
    try:
